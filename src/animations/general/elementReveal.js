    ...
    sections.forEach(section => {
      ...
      ctx = gsap.context(() => {
        ...
        elements.forEach(element => {
          ...
          const ease = element.getAttribute('anm-ease') || 'power2.inOut'
          ...
          switch (type) {
            ...
            case 'opacity':
              ...
              break
            case 'slide':
              const distance = element.getAttribute('anm-distance') || '100%'
              switch (direction) {
                case 'top':
                  tl.fromTo(element, { y: -distance, opacity: 0, ...animationProps }, { y: 0, opacity: 1, duration: duration, ease: ease, delay: delay, ...toStateProps }, 0)
                  break
                case 'bottom':
                  tl.fromTo(element, { y: distance, opacity: 0, ...animationProps }, { y: 0, opacity: 1, duration: duration, ease: ease, delay: delay, ...toStateProps }, 0)
                  break
                case 'left':
                  tl.fromTo(element, { x: -distance, opacity: 0, ...animationProps }, { x: 0, opacity: 1, duration: duration, ease: ease, delay: delay, ...toStateProps }, 0)
                  break
                case 'right':
                  tl.fromTo(element, { x: distance, opacity: 0, ...animationProps }, { x: 0, opacity: 1, duration: duration, ease: ease, delay: delay, ...toStateProps }, 0)
                  break
                case 'top left':
                  tl.fromTo(
                    element,
                    { x: -distance, y: -distance, opacity: 0, ...animationProps },
                    { x: 0, y: 0, opacity: 1, duration: duration, ease: ease, delay: delay, ...toStateProps },
                    0
                  )
                  break
                case 'top right':
