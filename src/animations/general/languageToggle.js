...
function init() {
  const languageToggle = document.querySelector('[anm-toggle=language]')
  const enText = document.querySelector('[anm-toggle=en-text]')
  const arText = document.querySelector('[anm-toggle=ar-text]')
  const toggleDot = document.querySelector('.btn-toggle__toggle-dot')
  ...
  ctx = gsap.context(() => {
    languageToggle.addEventListener('click', () => {
      ...
      gsap.to(toggleDot, {
        ...
        rotate: 0.001,
        onComplete: () => {
          // Wait additional 100ms after animation completes
          setTimeout(() => {
